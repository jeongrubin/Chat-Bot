...
- OpenAI API 키 없이도 동작하는 로컬 검색 모드
- API 키가 있으면 검색 결과만 근거로 답변하는 선택적 AI 답변 모드
...
로컬 TF-IDF 검색
    ↓
관련 프로그램 상위 3개 추출
    ├─ API 키 없음 → 구조화된 검색 결과 출력
    └─ API 키 있음 → 검색 결과를 근거로 AI 답변 생성
```
...
OpenAI 기반 자연어 답변을 사용하려면 `.env.example`을 `.env`로 복사한 뒤 본인의 API 키를 설정합니다.
